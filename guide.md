### 清楚所有终端：
pkill -9 -f falcon_platform 2>/dev/null; pkill -9 -f semi-party 2>/dev/null; pkill -9 -f "executor/falcon" 2>/dev/null; lsof -ti :30004-30007 2>/dev/null | xargs kill -9 2>/dev/null; sleep 1; echo "cleaned"
...
### 提交任务，返回任务 ID
cd /home/merak/falcon && python3 examples/coordinator_client.py -url 127.0.0.1:30004 -method submit -path examples/3party/dsls/examples/train/12.train_mlp_dp_clip.json 2>&1

### 查询任务进度
cd /home/merak/falcon && for attempt in $(seq 1 200); do sleep 15; status=$(python3 examples/coordinator_client.py -url 127.0.0.1:30004 -method query_status -job 1 2>&1); echo "[$(date +%H:%M:%S)] $status"; if [ "$status" = "finished" ] || [ "$status" = "failed" ]; then break; fi; done
